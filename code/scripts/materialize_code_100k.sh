...
OUTPUT_ROOT=${OUTPUT_ROOT:-$ROOT_DIR/data/derived/code_100k}
...
MAX_TOTAL_TOKENS=${MAX_TOTAL_TOKENS:-6144}
...
if [[ ! -e "$TOKENIZER_MODEL_NAME" && "$TOKENIZER_MODEL_NAME" == "Qwen/Qwen3-4B" && -n "$LOCAL_TOKENIZER_FALLBACK" && -d "$LOCAL_TOKENIZER_FALLBACK" ]]; then
  echo "[materialize_code_100k] tokenizer fallback: using local tokenizer at $LOCAL_TOKENIZER_FALLBACK for length filtering"
...
if [[ -f "$INPUT_JSONL" ]]; then
  ...
  echo "[materialize_code_100k] packaged source missing; using local cache fallback at $LOCAL_CACHE_FALLBACK"
  ARGS+=(--input_jsonl "$LOCAL_CACHE_FALLBACK")
else
  echo "[materialize_code_100k] local source not found at $INPUT_JSONL; falling back to source_url download path in builder"
fi
