...
class DistilConfig(TrainingArguments):
    ...
    cdsdft_enable: bool = field(
        default=False,
        metadata={"help": "Enable CD-SDFT loss."},
    )
    cdsdft_delta_metric: str = field(
        default="kl",
        metadata={"help": "Counterfactual delta metric. Only 'kl' is supported."},
    )
    cdsdft_delta_threshold: float = field(
        default=0.02,
        metadata={"help": "Delta threshold used in the CD-SDFT gate."},
    )
    cdsdft_retention_weight: float = field(
        default=0.02,
        metadata={"help": "Weight for KL(counterfactual || student)."},
    )
    cdsdft_gate_temperature: float = field(
        default=10.0,
        metadata={"help": "Temperature for the CD-SDFT token gate."},
    )
    cad_enable: bool = field(
        default=False,
        metadata={"help": "Enable CAD loss."},
    )
    cad_delta_metric: str = field(
        default="kl",
        metadata={"help": "Counterfactual delta metric for CAD. Only 'kl' is supported."},
    )
    cad_delta_threshold: float = field(
        default=0.02,
        metadata={"help": "Delta threshold used in the CAD gate."},
    )
    cad_gate_temperature: float = field(
        default=10.0,
        metadata={"help": "Temperature for the CAD token gate."},
    )
    cad_local_retention_weight: float = field(
        default=0.02,
        metadata={"help": "Weight for the local counterfactual retention term in CAD."},
    )
    cad_advantage_min: float = field(
        default=0.0,
        metadata={"help": "Lower clip for CAD advantage weights."},
    )
    cad_advantage_max: float = field(
        default=2.0,
        metadata={"help": "Upper clip for CAD advantage weights."},
    )
    cad_value_max: float = field(
        default=2.0,
        metadata={"help": "Normalization scale for CAD verifier values."},
    )
    osdft_enable: bool = field(
        default=False,
        metadata={"help": "Enable Orthogonal Gradient SDFT."},
    )
    osdft_acquisition_weight: float = field(
        default=1.0,
        metadata={"help": "Weight on the acquisition gradient in Orthogonal Gradient SDFT."},
    )
    osdft_preservation_weight: float = field(
        default=1.0,
        metadata={"help": "Weight on the preservation gradient in Orthogonal Gradient SDFT."},
    )
    osdft_preservation_source: str = field(
        default="counterfactual",
        metadata={"help": "Preservation target used by Orthogonal Gradient SDFT: 'counterfactual' or 'base'."},
    )
    osdft_project_all: bool = field(
        default=False,
        metadata={"help": "Project the acquisition gradient for all steps, not only when cosine conflict is negative."},
    )
    dualsdft_enable: bool = field(
        default=False,
        metadata={"help": "Enable DualSDFT mixed-target distillation over partial/full teacher views."},
    )
    dual_delta_threshold: float = field(
        default=0.02,
        metadata={"help": "KL(full || partial) threshold used in the DualSDFT gate."},
    )
    dual_gate_temperature: float = field(
        default=10.0,
        metadata={"help": "Temperature for the DualSDFT partial-to-full gate."},
    )
    dual_alpha_floor: float = field(
        default=0.10,
        metadata={"help": "Minimum full-view interpolation weight in DualSDFT."},
    )
    dual_alpha_cap: float = field(
        default=0.90,
        metadata={"help": "Maximum full-view interpolation weight in DualSDFT."},
    )
    dual_confidence_power: float = field(
        default=1.0,
        metadata={"help": "Exponent applied to the full-view top1-top2 margin confidence in DualSDFT."},
    )
    gdsdft_enable: bool = field(
        default=False,
        metadata={
            "help": "Enable simplified GD-SDFT, a single-lambda dual-view variant where partial is the baseline and full is scaled as a residual correction."
        },
    )
    gdsdft_lambda: float = field(
        default=1.0,
        metadata={
            "help": "Residual scaling in GD-SDFT: <1 is conservative toward partial, 1 matches full, >1 extrapolates beyond full."
        },
    )
    gdsdft_residual_clip: float = field(
        default=5.0,
        metadata={
            "help": "Absolute clip applied to log q_full - log q_partial before GD-SDFT residual scaling."
        },
    )
    dual_gopd_enable: bool = field(
        default=False,
        metadata={
            "help": "Use a G-OPD-inspired utility gate for DualSDFT, where partial is the baseline view and full is an extrapolative view."
        },
    )
    dual_gopd_lambda: float = field(
        default=1.25,
        metadata={
            "help": "Extrapolation factor applied to the DualSDFT full-minus-partial residual when dual_gopd_enable is set."
        },
    )
    dual_gopd_advantage_threshold: float = field(
        default=0.0,
        metadata={
            "help": "Threshold applied to the token-level log q_full(y_t) - log q_partial(y_t) advantage before the sigmoid gate in Dual G-OPD."
        },
    )
    dual_gopd_residual_clip: float = field(
        default=5.0,
        metadata={
            "help": "Absolute clip applied to log q_full - log q_partial before extrapolative target construction in Dual G-OPD."
        },
    )
    dual_teacher_cpu_offload: bool = field(
        default=False,
        metadata={
            "help": "Serialize DualSDFT full/partial teacher log-prob tensors through CPU so the two teacher views "
            "do not stay resident on GPU at the same time."
        },
    )
    dual_selected_logps_only: bool = field(
        default=False,
        metadata={
            "help": "Single-GPU survival mode for GD-SDFT: never materialize full-vocab teacher log-probs and "
            "distill only on the selected completion tokens."
        },
    )
    dual_exact_chunked_loss: bool = field(
        default=False,
        metadata={
            "help": "Single-GPU exact-ish GD-SDFT path: avoid full-vocab student log-prob tensors and compute "
            "the original vocab-level KD losses chunk-by-chunk."
        },
    )
    dual_terminal_gate_enable: bool = field(
        default=False,
        metadata={
            "help": "Replace the KL-delta DualSDFT gate with an answer-anchored terminal-similarity gate."
        },
    )
    dual_terminal_confidence_temperature: float = field(
        default=5.0,
        metadata={
            "help": "Temperature used when mapping full-view terminal similarity to confidence in terminal-gated DualSDFT."
        },
    )
    dual_terminal_advantage_threshold: float = field(
        default=0.0,
        metadata={
            "help": "Threshold applied to the full-vs-partial terminal-similarity advantage before the alpha sigmoid."
        },
    )
    opd_metrics_enable: bool = field(
        default=True,
        metadata={
            "help": "Log OPD-style student-vs-teacher process metrics when full-vocab log-probs are available."
        },
    )
    opd_metrics_topk: int = field(
        default=16,
        metadata={
            "help": "Top-k size used for OPD overlap metrics such as overlap_ratio and overlap_token_advantage."
        },
    )
